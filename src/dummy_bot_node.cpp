...
public:
	...
	static geometry_msgs::TransformStamped makeTransform(const ros::Time& stamp, const std::string& parent,
																											 const std::string& child, const double x, const double y,
																											 const double th)
	{
	...
	void timerUpdateCallback(const ros::TimerEvent& event)
	{
		...
		tf_pub_->sendTransform(makeTransform(event.current_real, parent_frame_, child_frame_, x_, y_, th_));
